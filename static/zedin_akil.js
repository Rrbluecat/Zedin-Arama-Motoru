 ...
 * [GÜNCELLEME]: Gelişmiş Türkçe Arama Normalizerı ve Kelime Eşleştirme Motoru Eklendi.
 */
...
// 🚀 [YENİ]: TÜRKÇE KARAKTER VE BÜYÜK/KÜÇÜK HARF SAĞIRLAŞTIRICI (Normalizer)
// Google mantığı: Arama sorgusu ve indeks verisi kıyaslanmadan önce bu süzgeçten geçer.
function zedinMetniStandardizeEt(metin) {
    if (!metin) return "";
...
// 🚀 [YENİ]: GELİŞMİŞ ALAKALILIK MOTORU (Full-Text Token Matcher)
function alakaliMi(sorguKelimeleri, baslik, icerik) {
    const normalizeBaslik = zedinMetniStandardizeEt(baslik);
    const normalizeIcerik = zedinMetniStandardizeEt(icerik);
    const birlesikMetin = normalizeBaslik + " " + normalizeIcerik;
    
    // Kelimelerden en az biri veya hepsi metinde geçiyor mu kontrolü
    for (let kelime of sorguKelimeleri) {
        if (kelime.length > 1 && !birlesikMetin.includes(kelime)) {
            return false; // Kelimelerden biri bile eksikse elenir (Google Katı Modu)
        }
    }
    return true;
}
