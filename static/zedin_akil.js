/**
 * 🧠 ZEDIN AKIL MOTORU (zedin_akil.js)
...
async function zedinAI_HizliYanitUret(sorgu, aramaSonuclari) {
    if (!ZedinAkılAyarları.modelYuklendimi || !zedinYZEngine) {
        return "Yapay zeka modeli arka planda yükleniyor, lütfen az sonra tekrar deneyin...";
    }
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    aiStatus.style = "position:fixed; top:0; left:0; width:100%; background:#f59e0b; color:white; text-align:center; font-size:12px; padding:6px; z-index:9999; font-family:sans-serif; font-weight:bold; box-shadow:0 2px 5px rgba(0,0,0,0.2);";
    aiStatus.innerText = "🧠 Zedin AI: Model arka planda yükleniyor (150MB)...";
    ...
    zedinAI_Baslat().then(() => {
        if (ZedinAkılAyarları.modelYuklendimi) {
            ...
            // 4 saniye sonra ekrandan otomatik kaybolur, arayüzü engellemez
            setTimeout(() => aiStatus.remove(), 4000);
        } else {
            aiStatus.style.background = "#dc2626"; // Hata durumunda kırmızı yap
            aiStatus.innerText = "❌ Zedin AI: Model yüklenirken bir hata oluştu!";
        }
    });
});
